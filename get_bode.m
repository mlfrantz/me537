function [freq_voltage, gain_table, phase_angle_table, gain_motor, phase_angle_motor] = get_bode(pos_motor, pos_table, voltage)
...
Y_voltage = fft(voltage(1:L2));
P2 = abs(Y_voltage/L);
P1 = P2(1:L/2+1);
P1(2:end-1) = 2*P1(2:end-1);
f = Fs*(0:(L/2))/L;

[max_val_voltage, ind] = max(P1(10:end)); % Highest non zero frequency
freq_voltage = f(ind); % Actual output frequency

Y_table = fft(pos_table(1:L2));
P2 = abs(Y_table/L);
P1 = P2(1:L/2+1);
P1(2:end-1) = 2*P1(2:end-1);
f = Fs*(0:(L/2))/L;
